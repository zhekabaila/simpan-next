...
export default function PengajuanPage() {
  ...
  const [pengajuanStatus, setPengajuanStatus] = useState<any[]>([])
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        setLoading(true)
        setError('')
        const result = await masyarakatService.getAllPengajuan(token)
  ...
  return (
    <div className="max-w-2xl mx-auto p-4 py-6">
      ...
      <div className="flex items-center justify-between mb-5">
        ...
        {pengajuanStatus.length === 0 || pengajuanStatus.some((p: any) => p.status === 'ditolak') ? (
          <button
      ...
      {pengajuanStatus.length === 0 ? (
        ...
        <div className="space-y-3">
          {pengajuanStatus.map((pengajuan: any, index: number) => (
            <div key={index} className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5">
              <div className="flex items-start justify-between mb-3">
                <div>
                  <p className="font-extrabold text-slate-800 tracking-wider text-base">{pengajuan.nomor_pengajuan}</p>
              ...
              <div className="flex items-center gap-4 text-xs text-slate-400">
                <div className="flex items-center gap-1.5">
                  ...
                  <span>{formatUTCDate(pengajuan.diajukan_pada, 'date')}</span>
                </div>
              </div>
              {pengajuan.catatan_admin && (
                <div className="mt-3 p-3 bg-amber-50 rounded-xl">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs text-amber-700 font-medium">{pengajuan.catatan_admin}</span>
                  </div>
                </div>
              )}
              <div className="mt-3">
                ...
                <div className="flex items-center gap-2">
                  {['Dikirim', 'Ditinjau', 'Diputuskan'].map((step, i) => {
                    const statusIndex = pengajuan.status === 'menunggu' ? 0 : pengajuan.status === 'ditinjau' ? 1 : 2
                    return (
              ...
              {pengajuan?.status === 'ditolak' && (
                <Link
                  ...
                  className="flex justify-center items-center gap-3 text-base font-medium bg-blue-600 hover:bg-blue-500 text-white px-2.5 py-1.5 rounded-md mt-4">
